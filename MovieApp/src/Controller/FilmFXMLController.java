 ...
 * @author Hellen_User
 */
public class FilmFXMLController implements Initializable, ControllerWithIdentificator {
    ...
    void onClickCreateButton(ActionEvent event) {
        ...
        boolean res = facade.createFilm(login, name, info, type, cost);
        int ress = -6;
        if(res==true)
            ress = 0;
        switch (ress) {
    ...
    void onNamechange3(MouseEvent event) throws IOException {
        ...
        ArrayList<String> res = facade.getFilmInfodop2(txtName.getText());
    ...
    void onClickAccesPayButton(ActionEvent event) {
            if(facade.filmAcessbuy(login, idFilm)){
    ...
    void onClickCashierPayButton(ActionEvent event) {
        if(facade.filmToCashier(login, idFilm)){
            lblAnswer.setText("Success");
            btnWantPay.setVisible(false);
        }else
            lblAnswer.setText("Somsing wrong");
    ...
    void onClickCreateSeansButton(ActionEvent event) {
        ...
        lblAnswer.setText(facade.CreateSeans(login, idFilm, dateSeans, txtHour.getText(), txtMinutes.getText()));
    }

    @FXML
    void onClickPayButton(ActionEvent event) {
            if(facade.filmPay(login, idFilm)){
            lblAnswer.setText("Success");
            btnWantPay.setVisible(false);
        }else
            lblAnswer.setText("Somsing wrong");
    ...
    void onClickWantPayButton(ActionEvent event) {
        if(facade.filmWantBuy(login, idFilm)){
            lblAnswer.setText("Success");
            btnWantPay.setVisible(false);
        }else
            lblAnswer.setText("Somsing wrong");
    ...
    @Override
    public void init(String login, String identificator2, String filmid) {
        facade = Facade.getInstance();
        this.login = login;
        this.idFilm = filmid;
        String type = facade.getPersonType(login);
        if(type.equals("KinoKompany")){
            ...
            if(identificator2.equals("")){
                ...
                btnCreateSeans.setVisible(false);
                try {
                    txtStatusFilm.setText(facade.getFilmStatus(filmid));
                } catch (IOException ex) {
                    Logger.getLogger(FilmFXMLController.class.getName()).log(Level.SEVERE, null, ex);
                }
                try {
                    ArrayList<String> res = facade.getFilmInfo(filmid);
