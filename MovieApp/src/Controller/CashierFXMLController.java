 ...
 * @author Hellen_User
 */
public class CashierFXMLController implements Initializable , ControllerWithIdentificator{
            ...
            row.setOnMouseClicked(new EventHandler<MouseEvent>() {
                ...
                public void handle(MouseEvent event) {
                    if (event.getClickCount() == 2 && (!row.isEmpty())) {
                        ...
                        lblAnswer.setText(facade.PayBron(login, String.valueOf(rowData.getId())));
                        //MovieApp.FilmView(login,"",Integer.toString(rowData.getId()));
                        //MovieApp.createCustomerTripView(Integer.toString(rowData.getId()));
                    }
                }
