...
public class PersBuyJUnitTest {
    ...
    // Бизнес процесс бронирование билетов
     ...
     int filmProkatCost =5000;
        ...
        assertTrue(kinkomp.addFilm(filmProkatCost, name, info, timef));

        assertEquals(name, rep.getFilms().get(0).getName());
        rep.getFilms().get(0).setStatus(FilmStatus.Payed);
        rep.updateAll();
        ...
        assertTrue(manager.NewSeans(time, rep.getFilms().get(0)));
        assertFalse(manager.NewSeans(timebad1, rep.getFilms().get(0)));
        assertFalse(manager.NewSeans(timebad2, rep.getFilms().get(0)));
        assertTrue(manager.NewSeans(timegood, rep.getFilms().get(0)));
        //Seans se = new Seans(time, rep.getFilms().get(0));
